function recordNoise_soundcard(speaker, stimDur, scale, configFile)
...
% A) Hardware
    ...
    % 3) A MATLAB-compatible sound card. 
...
% B) Software
    ...
    % 2) Hardware class definitions and databases, all available at:
    % https://github.com/danieldkato/hardware_tests/tree/master/test_speakers/get_speaker_spectrogram
...
dirName = strcat(['spkr',rename(speaker), '_white_noise_', startTime, '_mic', rename(Recording.Microphone), '_sigCond', rename(Recording.SignalConditioner)]);
mkdir(dirName);
old = cd(dirName);
